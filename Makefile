...
all: ingest reference coverage qc qc-report spatial features dataset baselines model evaluation
...
model:
	uv run python -m src.eval.run_model

evaluation:
	uv run python -m src.eval.figures
	uv run python -m src.eval.run_full_eval
